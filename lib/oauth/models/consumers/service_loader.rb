...
# We use our own service loader to give priority to rails models
if defined? ConsumerToken && defined? OAUTH_CREDENTIALS
  ...
  OAUTH_CREDENTIALS.each do |key, value|
    ...
    unless Object.const_defined?(class_name.to_sym)
      # check if we have the file in our models folder
      begin
        # Let Rails auto-load from the models folder
        eval class_name
      rescue NameError
        # next try to load from our services
